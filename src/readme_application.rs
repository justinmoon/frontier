...
use crate::navigation::{execute_fetch, prepare_navigation, FetchRequest, FetchedDocument};
...
impl ReadmeApplication {
    ...
    fn compose_html(&self) -> String {
        ...
        if let Some(document) = &self.current_document {
            crate::wrap_with_url_bar(&document.contents, &self.current_input)
        } else {
            crate::wrap_with_url_bar("<p>Loading…</p>", &self.current_input)
        }
    }
    ...
    fn build_document_with_chrome(&self, contents: &str, base_url: &str) -> HtmlDocument {
        let html = crate::wrap_with_url_bar(contents, &self.current_input);
...
async fn run_fetch_task(
    ...
    match execute_fetch(&request, Arc::clone(&net_provider)).await {
        Ok(document) => {
